...
public:
    ...
    AbstractSolver(const AdjacencyMatrix &adj_matrix, EPS eps_prune, int turn_mode, int turn_cost, const LoggerPtr logger): adj_matrix(adj_matrix), eps_prune(eps_prune), turn_mode(turn_mode), turn_cost(turn_cost), logger(logger) {}
    AbstractSolver(const AdjacencyMatrix &adj_matrix, EPS eps_merge, EPS eps_prune, int turn_mode, int turn_cost, const LoggerPtr logger): adj_matrix(adj_matrix), eps_prune(eps_prune), eps_merge(eps_merge), turn_mode(turn_mode), turn_cost(turn_cost), logger(logger) {}
