...
std::string output_file = "../output_log_2min.txt";
...
int main() {
    std::vector<std::string> algorithms = {"BBMOCBS-k -k 5"};//{"LCBS -k 1", "BBMOCBS-k -k 1", "BBMOCBS-eps", "BBMOCBS-pex"};
    std::vector<int> agent_counts = {5, 10, 15, 20, 25, 30, 35};
    int total_runs = 25;

    std::ofstream summary("../sim_data_algos_2min.txt", std::ios::app);
    ...
    for (const auto& algorithm : algorithms) {
        for (int agents : agent_counts) {
            ...
            for (int i = 1; i <= total_runs; ++i) {
                std::cout << "\033[1;34m" << "[2 min]Running scenario " << i << " for algorithm: " << algorithm << "\033[0m" << std::endl;
