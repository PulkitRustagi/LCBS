...
std::string make_command(const std::string& scen_file, int agent_num, const std::string& algorithm, const std::string& map_name) {
    ...
    std::string output_file = "../data/"+map_name+"_output_log_4D_"+time_lim+"sec.txt";
...
void write_run_outcomes(const std::string& output_file, const std::string& algo_label, int agents, const std::string& map_name) {
    std::ifstream infile(output_file);
...
int main() {
    std::vector<std::string> map_names =   {"random-64-64-10"};
    ...
    for (const auto& map_name : map_names) {   
        std::string output_file = "../data/"+map_name+"_output_log_4D_"+time_lim+"sec.txt";
        std::ofstream summary("../data/rand_scen_results/results_rand_scen_"+map_name+"_"+time_lim+"sec_4D.txt", std::ios::app);
        ...
        for (const auto& algorithm : algorithms) {
            for (int agents : agent_counts) {
                ...
                for (int i = 1; i <= total_runs; ++i) {
                    ...
                    std::string command = make_command(scen_name.str(), agents, algorithm, map_name);
                    std::system(command.c_str());
                }
                
                write_run_outcomes(output_file, algorithm, agents, map_name);
